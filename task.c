...
void pop()
{
  if (q->head != NULL)
  {
    printf("__id %d__total: %d___", q->head->id, q->total);
    printf("__id of next: %d__\n", q->head->next->id);
...
{
  task *temp = q->head;
  task *freeMe = malloc(sizeof(task));
  int count = 0;
  while (temp->next != NULL)
  {
    pthread_mutex_lock(&lock);
    printf("id %d__total: %d___", q->head->id, q->total);
    printf("__id of next: %d__\n", q->head->next->id);
    freeMe = temp;
    temp = temp->next;
    free(freeMe);
...
void *doTasks(void *k)
{
  ...
  else
  {
    ...
    {
      pthread_mutex_lock(&lock);
      while (q->head->next != NULL && q->head->id != q->total)
      {
        task *current = q->head;
        q->head = q->head->next;
        free(current);
...
int main(int argc, char *argv[])
{
  ...
  stop = clock();
  double span = (double)(stop - start) / CLOCKS_PER_SEC;
